...
public struct StableDiffusionConfiguration: Sendable {
    ...
    /// See https://huggingface.co/stabilityai/sd-turbo for the model details and license.
    /// SD-Turbo is distilled SD 2.1 (single CLIP text encoder, smaller UNet than SDXL) —
    /// same architecture/file layout as the 2.1-base preset, with a 1-step / cfg-0 schedule.
...
/// Spike: pre-download an external VAE repo (e.g. madebyollin/sdxl-vae-fp16-fix) so
/// `loadAutoEncoder` can load it from the local cache. Call before building the generator.
public func downloadVAERepo(_ repoId: String, hub: HubApi = HubApi()) async throws {
    try await hub.snapshot(
        from: Hub.Repo(id: repoId),
        matching: ["config.json", "diffusion_pytorch_model.safetensors"]) { _ in }
}
    ...
    -> Autoencoder
{
    ...
    if let vaeRepo = (vaeRepoEnv ?? defaultVAERepo).isEmpty ? nil : (vaeRepoEnv ?? defaultVAERepo) {
        ...
        let cfg = try JSONDecoder().decode(
            ...
            from: Data(contentsOf: dir.appending(component: "config.json")))
        let model = Autoencoder(configuration: cfg)
        try loadWeights(
            url: dir.appending(component: "diffusion_pytorch_model.safetensors"),
            model: model, mapper: vaeRemap, dType: .float16)
        return model
    }
